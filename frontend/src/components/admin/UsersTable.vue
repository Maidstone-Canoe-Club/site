...
<template>
  <div class="mt-8">
    <div class="px-4 sm:px-6 lg:px-8">
      <div class="sm:flex sm:items-center justify-between gap-2">
          ...
          <tr v-for="user in users" :key="user.id">
            <td class="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-0">
              ...
              <dl class="font-normal lg:hidden">
                ...
                <dd class="mt-1 truncate text-gray-500 sm:hidden">Last access {{ timeSinceLastLogin(user) }}</dd>
              </dl>
            </td>
            <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">
              {{ user.email }}
            </td>
            <td class="px-3 py-4 hidden sm:table-cell text-sm text-gray-500">{{ timeSinceLastLogin(user) }}</td>
